...
这是与存档编辑器分离的 Windows Target，发布文件名为 `仙剑98存档检查工具.exe`。推荐部署到游戏目录的 `Tools/PalSaveChecker` 独立子目录；双击后会自动越过 `Tools` 识别游戏根目录并执行一次检查，也可随时点击“检查”或“修复”。旧版直接放在 `Tools` 下的布局继续兼容。
...
7. 如果存在 `N.RPG.pal98-custom-roles.json`，同时读取游戏根目录固定 `palmod/CustomRoles/roles.json`。RPG 文件名、大小、SHA-256、库 ID/版本和每条角色状态载荷仍严格校验；只有从角色 6 开始的连续尾部角色数量变化可判定为安全修复。
...
自定义主角 sidecar 修复遵循新版编辑器同一合同：固定库只在末尾追加角色时，已有共同前缀角色的全部字段、经验和修正量原样保留，新角色按固定库 `initial_state` 与等级领悟表初始化；固定库只从末尾删除角色时，保留仍存在的共同前缀并裁掉已删除尾部状态。以“苗胖（角色 6）→ 追加鼻祖（角色 7）”为例，修复只补入鼻祖，不重置苗胖。角色重排、稀疏编号、RPG 哈希、库 ID/版本或 JSON 结构不符均拒绝自动覆盖。保留备份时，原自定义主角 sidecar 保存为对应 RPG 备份名加 `.pal98-custom-roles.json`；修复后必须重读并确认不再需要调和。
