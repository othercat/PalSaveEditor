...
v0.1.6（2026-09-13）：为 PALDLL v1.63 完整包重新发布当前编辑器与检查工具，包含 v1.62 原生技能槽、全局技能名称和多文件存档回滚修复。两个工具及配套 Core DLL 统一为 0.1.6.0。

2026-09-07 工具维护版：识别 v1.62 固定技能 2048 起的正式名称，原版六人限定为 RPG 中的 32 个物理技能槽；检查工具按完整运行时对象表校验角色 6–15 的领悟表。v1.62 下不激活或重写旧 999 槽附属文件。编辑器在后续附属文件写入失败时同时恢复 RPG、角色快照及角色库，避免只恢复 RPG 后产生身份失配。
